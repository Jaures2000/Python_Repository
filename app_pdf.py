...
from export_pdf import generer_pdf_utilisateur, generer_pdf_tous_patrimoines
...
@app.route("/exporter_mes_patrimoines")
def exporter_mes_patrimoines():
    if "id_user" not in session:
        return redirect(url_for("login"))
    
    id_user = session["id_user"]
    nom_utilisateur = session.get("nom_utilisateur", "Utilisateur")
    
    # Récupérer SES patrimoines uniquement
    patrimoines = recuperer_patrimoines_par_user(id_user)
    
    if not patrimoines:
        flash("Vous n'avez aucun patrimoine à exporter", "warning")
        return redirect(url_for("index"))
    ...
    # Générer le PDF
    nom_fichier = f"exports/mes_patrimoines_{nom_utilisateur}.pdf"
    generer_pdf_utilisateur(patrimoines, nom_utilisateur, nom_fichier)
    
    return send_file(nom_fichier, as_attachment=True, 
                     download_name=f"mes_patrimoines.pdf")


@app.route("/exporter_tous_patrimoines")
def exporter_tous_patrimoines():
    if "id_user" not in session:
        return redirect(url_for("login"))
    
    # Récupérer TOUS les patrimoines
    tous_patrimoines = recuperer_patrimoines()
    
    if not tous_patrimoines:
        flash("Aucun patrimoine dans la base", "warning")
        return redirect(url_for("index"))
    ...
    nom_fichier = "exports/tous_patrimoines.pdf"
    generer_pdf_tous_patrimoines(tous_patrimoines, nom_fichier)
    
    return send_file(nom_fichier, as_attachment=True, 
                     download_name="tous_patrimoines.pdf")
